...
file_path <- "G:/Projects/Euroleague Fantasy/Data/PlayersStats_edit.csv"
players_stats <- read.csv(file_path, stringsAsFactors = FALSE)

# 2. Επισκόπηση των δεδομένων
str(players_stats)      # Τύποι στηλών
head(players_stats)     # Δείγμα δεδομένων
...
players_stats <- players_stats %>%
  mutate(
    TotalSeconds = sapply(Minutes, convert_to_seconds)
  )

# 5. Επισκόπηση του νέου dataframe
str(players_stats)      # Νέοι τύποι στηλών
head(players_stats)     # Δείγμα δεδομένων

# 6. Αποθήκευση σε νέο CSV (προαιρετικά)
write.csv(players_stats, "G:/Projects/Euroleague Fantasy/Data/PlayersStats_Updated.csv", row.names = FALSE)
